...
const remaining = computed(() => Math.max(props.mentor.capacity - props.taken, 0));
const full = computed(() => remaining.value <= 0);
const tags = computed(() =>
  String(props.mentor.expertise || "")
    .split(/[、,，/]/)
    .map((s) => s.trim())
    .filter(Boolean)
);
...
<template>
  <article class="card" :class="{ mine, full: full && !mine }">
    ...
    <div class="quota" :class="{ full }">
      <strong>{{ taken }} / {{ mentor.capacity }}</strong>
      <span>{{ full ? "名额已满" : "剩余 " + remaining + " 个名额" }}</span>
    </div>

    <dl class="meta">
      <div v-if="mentor.industry">
        <dt>行业领域</dt>
        <dd>{{ mentor.industry }}</dd>
      </div>
      <div v-if="mentor.timePref">
        <dt>辅导时间</dt>
